...
template<typename T>
struct ObjectSlot {
    ObjectSlot(ID id_, T* object_) : id(id_), object(object_) {}
...
struct ObjectSlotConst {
    ObjectSlotConst(ID id_, const T* object_) : id(id_), object(object_) {}
...
struct PRef
{
    ...
    template<typename U>
    PRef(ID index, Vector<U>* a, ID vid): id(index), provider_callback{PRef<T>::get<U>}, provider(a), validity_id(vid) {}

    template<typename U>
    PRef(const PRef<U>& other) : id(other.id), provider_callback{PRef<T>::get<U>}, provider(other.provider), validity_id(other.validity_id) {}
